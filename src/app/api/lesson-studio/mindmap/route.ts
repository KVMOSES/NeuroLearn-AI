 ...
 * Generate an interactive mind map from a document.
 */
import { getSession } from "@/lib/session";
import { ok, ApiError } from "@/lib/api";
import { generateMindMap } from "@/lib/lesson-studio";
import { z } from "zod";
...
export async function POST(req: Request) {
  ...
  try {
    const mindMap = await generateMindMap(parsed.data.documentId);
    return ok({ mindMap });
  } catch (err) {
